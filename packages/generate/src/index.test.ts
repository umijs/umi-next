...
test('generate tpl', async () => {
  ...
  expect(
    existsSync(join(cwd, 'hello', 'index.tsx')),
  ).toEqual(true);
...
test('generate tpl file', async () => {
  ...
  expect(
    existsSync(join(cwd, 'file-tpl', 'index.tsx')),
  ).toEqual(true);
...
test('generate by file', async () => {
  ...
  expect(
    existsSync(join(cwd, 'file', 'a.tsx')),
  ).toEqual(true);
...
test('generate tpl by data', async () => {
  ...
  expect(
    existsSync(join(cwd, 'data', 'index.tsx')),
  ).toEqual(true);
  expect(
    readFileSync(join(cwd, 'data', 'index.tsx'), 'utf-8')
  ).toContain('HomePage');

 ...
 * data-code-line="2" data-code-column="3" data-code-relative-path="xxx/ooo" 简化一下成代码
 * like <div data-find-code="["2","3","xxx/ooo"]" />
 * this props will be record in fiber
 */
export interface CodeDataAttribute {
  'data-find-code': string;
}
...
export const getCodeInfoFromProps = (fiber?: Fiber): CodeInfo | undefined => {
  ...
  const { 'data-find-code': findCodeString } =
    fiber.pendingProps as CodeDataAttribute;
  let findCodeArray = [];
  try {
    if (findCodeString && findCodeString.length > 0) {
      findCodeArray = JSON.parse(findCodeString);
    }
  } catch (error) {
    console.log(`JSON error : ${error}`);
  }
  if (findCodeArray && findCodeArray.length === 3) {
    return {
      lineNumber: findCodeArray[0],
      columnNumber: findCodeArray[1],
      relativePath: findCodeArray[2],
    };
  }

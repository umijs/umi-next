...
export const defaultHotKeys = ['control', 'option', 'command', 'i'];
...
export const Inspector: React.FC<InspectorProps> = (props) => {
  ...
  const handleInspectKey = () => (isInspect ? stopInspect() : startInspect());

  useEffect(() => {
    // @ts-ignore
    const handleHotKeys = (event: any, handler: { key: string }) => {
